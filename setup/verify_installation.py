...
def check_dependencies():
    ...
    required_packages = [
        "pydantic", "openai", "tenacity", "pyyaml", "loguru", 
        "numpy", "fastapi", "tiktoken", "browser-use", "aiofiles"
    ]
    ...
    if not all_installed:
        print_warning("Some required packages are missing. Run 'pip install -r requirements.txt'")
...
def check_browser_tools():
    ...
    try:
        ...
        if result.returncode == 0:
            print_success(f"Playwright is installed: {result.stdout.strip()}")
            return True
        else:
            print_warning("Playwright is not installed or not in PATH")
            print_warning("Run 'playwright install' for browser automation features")
            return False
    except FileNotFoundError:
        print_warning("Playwright command not found")
        print_warning("Run 'pip install playwright' and 'playwright install'")
        return False
